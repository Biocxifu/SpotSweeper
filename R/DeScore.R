...
DeScore <- function(object =object,test.method='wilcox.test',hide.ns = T,ctr.group=ctr.group,
  ...
  if(!ScoreType %in% c('ssGSEA','AddModuleScore')){
    stop('The input ScoreType is incorrect')
  }
  if(is.null(ctr.group)){
    stop('Ctr.group not set')
  }


  if (ScoreType=='ssGSEA') {
    cellscore <- object@assays[["ImmuneScore"]][['ssGSEA']]%>% t() %>% as.data.frame()}
